-- NOVA v6.1 SINGLE FILE | ESP / Aimbot / Misc (South Bronx auto-detect)
...
print("[NOVA] v6.1 boot (single file)")
...
print("[NOVA] v6.1 | game=" .. GAME_VERSION .. " place=" .. tostring(detectedPlace) .. " universe=" .. tostring(detectedUniverse))
...
local function deepCleanCharacter(char)
    ...
    for _,d in ipairs(char:GetDescendants()) do
        if d.Name=="ESPHL" or d.Name=="ESPBox" or d.Name=="ESPName" or d.Name=="ESPHealth" then
            pcall(function() d:Destroy() end)
        elseif d:IsA("Highlight") or d:IsA("BoxHandleAdornment") or d:IsA("SelectionBox") then
            pcall(function() d:Destroy() end)
        elseif d:IsA("BillboardGui") and (d.Name=="ESPBox" or d.Name=="ESPName" or d.Name=="ESPHealth") then
            pcall(function() d:Destroy() end)
        end
    end
end
...
loadVer.BackgroundTransparency=1 loadVer.Text="v6.1 ("..FILE_TAG..")"
...
w3.BackgroundTransparency=1 w3.Text="V 6.1 • "..GAME_VERSION w3.Font=Enum.Font.Gotham
...
statLbl.Text="NOVA v6.1 • "..GAME_VERSION statLbl.Font=Enum.Font.GothamBold
...
function saveConfig()
    ...
    local data={
        ...
        MaxESP=Settings.MaxESP, Health=Settings.Health, Tracers=Settings.Tracers, HeadDot=Settings.HeadDot,
...
function loadConfig()
    ...
    for _, id in ipairs({"ESPEnabled","Boxes","Names","Distance","TeamCheck","MaxESP","Health","Tracers","HeadDot",
...
local function createESP(player)
    ...
    ESPData[player]={boxGui=boxGui,stroke=stroke,hpFill=hpFill,hpBar=hpBar,nameGui=nameGui,nameLabel=label,parts=parts,hrp=hrp,hum=hum,tracer=tracer,headdot=headdot}
    creating[player]=nil
end
        ...
        if player~=LocalPlayer then
            ...
            elseif d.hrp.Parent==nil or d.hum.Parent==nil or d.boxGui.Parent==nil or d.nameGui.Parent==nil then
                ...
                local okP = pcall(function()
                    ...
                    d.hpBar.Visible=show and Settings.Health
...
local function finishLoading()
    ...
    LoadingDone=true
    print("[NOVA] v6.1 loaded ("..FILE_TAG.." / "..GAME_VERSION..")")
